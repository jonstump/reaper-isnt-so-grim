...
- `.aup3` is a SQLite database holding the project document plus audio sample blocks ([format background](https://liliputing.com/audacity-3-0-released-with-new-project-file-format-open-source-cross-platform-audio-editor/), official [audacity-project-tools](https://github.com/audacity/audacity-project-tools) shows recovery/extraction is feasible). **The document is not XML** — it's a tagged binary stream paired with a name dictionary, split across the `dict` and `doc` columns of a `project` table, with audio in `sampleblocks`. The `.aup` format it replaced *was* XML, which is where that assumption came from. Everything now known about the format, and where each fact came from, lives in [`importer/FORMAT.md`](importer/FORMAT.md).
- `.rpp` is plain text and well understood — generating one is the easy part. That leaves **two** hard halves, not one: decoding the project document and decoding the sample blocks. Both have a lawful reference implementation (see the clean-room note below), so neither is the dead end this plan originally feared — but the format has traps that produce plausible *wrong* output rather than errors, and those are catalogued in `FORMAT.md`. Read it before writing the converter, not after.
...
- Clean-room note: read the format by inspecting files and reading public prose — don't port Audacity's GPL source. Permissively-licensed third-party implementations *are* fair game, verified and attributed ([ADR-0005](docs/adrs/ADR-0005-license-and-clean-room.md), amended). `audacity-project-tools` turns out to be BSD-3-Clause rather than GPL, which is what unblocked both hard halves above.
