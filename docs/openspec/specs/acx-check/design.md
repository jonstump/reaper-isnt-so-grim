...
* ~~**What is the minimum room-tone window duration?**~~ **Resolved: 0.5 s.** Long enough for a stable RMS reading at any rate a narrator records at, short enough that a densely-read chapter with little silence still yields a qualifying window. Recorded as `source.MIN_ROOM_TONE_SEC`, with the reasoning at the constant. Still worth revisiting against narration sample D1 once it arrives — the constant is the only thing that would change.
* ~~**What advisory margin should trigger the true-peak line?**~~ **Resolved: 0.3 dB.** Recorded as `thresholds.acx.true_peak_advisory_margin`. Chosen as a middle value pending real-world feedback on whether it fires too often or too rarely — revisit once the report has seen real material.
...
* ~~**What is the exact minimum Reaper version** that provides `CalculateNormalization`?~~ **Resolved: 6.44.** Confirmed against REAPER's official changelog archive — "ReaScript: add CalculateNormalization function" first appears in v6.44 (2022-01-07) and is absent from every v6.30–v6.43 entry. See [spike 9](../../../spikes/issue-9.md#3-minimum-reaper-version-is-644-not-637). The version check required by REQ "Measurement Source and Version Floor" reports 6.44; ADR-0003, ADR-0004, and the README are corrected from the earlier guess of 6.37.
